...
However, in both slot 2 and 10, the average surplus difference may be within the range of -6 and 6, assuming the average room surplus is 3. See the first blog in the series [here]({{site.baseurl}}/_posts/2024-08-16-building-an-assignment-algorithm-1.markdown) for how the surplus difference is calculated. 
</details>
...
standardisedCompromiseScore = (mean surplus difference/max surplus) x (attendee Z score/2.72)^3 
...
if maxSurplus != 0 and attendee surplus difference >0: 

standardisedSurplusScore = attendee surplus difference/max surplus difference   (here max surplus has to be positive) 

  

if maxSurplus != 0 and attendee surplus difference <0: 

standardisedSurplusScore = attendee surplus difference/|min surplus difference|   (here min surplus has to be negative) 

  

if maxSurplus = 0         

standardisedSurplusScore= attendee surplus difference   
